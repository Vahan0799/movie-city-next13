...
import {getMovieGenres} from '@/services/genre';
import {setMovieGenres} from '@/redux/slices/genreSlice';
...
const Index = () => {
    ...
    const {movieGenreList} = useSelector(state => state.genre);
    ...
    useEffect(() => {
        if (movieGenreList.length === 0 || prevLocale !== locale) {
            getMovieGenres(locale)
                .then(response => dispatch(setMovieGenres(response)))
        }

    }, [movieGenreList, locale]);

    return (
        <header className={classNames([styles.header, (isDown && !showMovieGenres) ? styles.headerDown : ''])}>
